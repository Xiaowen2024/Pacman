...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState: busters.GameState):
        ...
        for ghostPositionDist in livingGhostPositionDistributions:
            ghostPositions.append(ghostPositionDist.argMax())
        closestGhost = ghostPositions[0]
        distance = float("inf")
        for pos in ghostPositions:
            if self.distancer.getDistance(pos,pacmanPosition) < distance:
        ...
        for action in legal:
            ...
            if self.distancer.getDistance(successorPosition,closestGhost) < nextDistance:
                nextDistance = self.distancer.getDistance(successorPosition,closestGhost)
                chosenAction = action
        return chosenAction
